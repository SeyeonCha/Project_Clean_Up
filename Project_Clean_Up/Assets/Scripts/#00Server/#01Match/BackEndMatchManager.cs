...
public partial class BackEndMatchManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ServerGameManager.OnGameReconnect += OnGameReconnect;
        // 핸들러 설정
        MatchMakingHandler();
        GameHandler();
        ExceptionHandler();
    ...
    private void GameHandler()
    {
        Backend.Match.OnSessionJoinInServer += (args) =>
        {
            ...
            if (args.ErrInfo != ErrorInfo.Success)
            {
                if (isReconnectProcess)
                {
                    ...
                    {
                        //재접속 성공
                        ServerGameManager.GetInstance().ChangeState(ServerGameManager.GameState.Reconnect);
                        Debug.Log("재접속 성공");
                    }
        ...
        Backend.Match.OnMatchResult += (args) =>
        {
            ...
            if (args.ErrInfo == BackEnd.Tcp.ErrorCode.Success)
            {
                ...
                ServerGameManager.GetInstance().ChangeState(ServerGameManager.GameState.Result);
            }
